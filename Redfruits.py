...
class HongguoPlugin:
    ...
    def searchContent(
        ...
        兼容紅果 API 不同版本可能出現的搜尋參數及回傳結構：
        1. wd / keyword / q 三種關鍵字參數。
        2. pg / page / pageNo / pageNum 四種分頁參數。
        3. list / data.list / result.list 等資料層級。
        4. 避免錯誤的空結果直接中斷後續參數嘗試。
        """
        del quick
        ...
        def _extract_rows(payload: Any) -> List[Any]:
            ...
            # 常見第一層
            for key_name in ("list", "items", "results", "videos", "vod"):
                value = payload.get(key_name)
                if isinstance(value, list):
                    return value

            # 常見第二層：data / result / data.data / result.data
            for outer in ("data", "result"):
                value = payload.get(outer)
                if isinstance(value, list):
                    return value
                if isinstance(value, Mapping):
                    for inner in ("list", "items", "results", "videos", "vod", "data", "result"):
                        rows = value.get(inner)
                        if isinstance(rows, list):
                            return rows
            return []

        # 以 wd/pg 為首選；若上游版本改名，依序嘗試其他常見格式。
        attempts = [
            {"wd": keyword, "pg": str(page)},
            {"keyword": keyword, "pg": str(page)},
            {"q": keyword, "pg": str(page)},
            {"wd": keyword, "page": str(page)},
            {"keyword": keyword, "page": str(page)},
            {"q": keyword, "page": str(page)},
            {"wd": keyword, "pageNo": str(page)},
            {"keyword": keyword, "pageNo": str(page)},
            {"wd": keyword, "pageNum": str(page)},
        ]
        ...
        for params in attempts:
            try:
                payload = self._api_fetch("/search", params)
                last_payload = payload if isinstance(payload, Mapping) else {}
                rows = _extract_rows(payload)
                if rows:
                    items = _content_items(rows)
                    if items:
                        break
            except Exception:
                continue

        # 某些版本搜尋 API 回傳 data/list，但內容 ID 欄位不同；
        # _content_items 已負責標準 vod_id/id 兼容，這裡不再改寫 ID，
        # 保證搜尋結果點擊 detailContent 時仍使用同一套 opaque ID。
        if not items and isinstance(last_payload, Mapping):
            rows = _extract_rows(last_payload)
            if rows:
                items = _content_items(rows)

        # 真實總數若由 API 提供則優先使用；否則採用 TVBox 可翻頁估算。
        total = None
        for obj in (last_payload,):
        ...
        page_count = max(page, (total + 17) // 18) if total > 0 else page
        if looks_full and page_count <= page:
            page_count = page + 1
